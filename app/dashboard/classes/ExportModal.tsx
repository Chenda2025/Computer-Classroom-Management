...
const COLUMNS = [
  { key: 'index', label: '#' },
  { key: 'classCode', label: 'លេខកូដ' },
  { key: 'name', label: 'ឈ្មោះថ្នាក់' },
  { key: 'academicYear', label: 'ឆ្នាំសិក្សា' },
  { key: 'maxStudents', label: 'សមត្ថភាពសិស្ស' },
] as const;
...
function cellValue(c: ClassRow, key: ColKey, idx: number): string {
  switch (key) {
    case 'index': return String(idx + 1);
    case 'maxStudents': return c.maxStudents ? String(c.maxStudents) : '—';
...
export default function ExportModal({ classes, onClose }: Props) {
  ...
  const [mainTitle, setMainTitle] = useState('បញ្ជីថ្នាក់រៀនសរុប');
  ...
  useEffect(() => {
    const saved = localStorage.getItem('exportSettings_classes');
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('exportSettings_classes', JSON.stringify(data));
  ...
  const buildAoa = () => [
    ...
    ...classes.map((s, i) => activeCols.map(c => cellValue(s, c.key, i))),
    [],
    ['សរុបថ្នាក់៖ ' + classes.length + ' ថ្នាក់'],
    [],
              ...
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{classes.length}</strong> ថ្នាក់</p>
                  </div>

...
function cellValue(r: ExamResultRow, key: ColKey, idx: number): string {
  ...
  switch (key) {
    ...
    case 'score': return String(r.score);
    case 'result': return passed ? 'ជាប់' : 'ធ្លាក់';
    case 'promoted': return r.promoted ? 'បានបន្តវគ្គ' : '—';
    case 'date': return new Date(r.createdAt).toLocaleDateString('km-KH', { year: 'numeric', month: 'short', day: 'numeric' });
    default: return '—';
  }
}
              ...
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{results.length}</strong> លទ្ធផល</p>
                  </div>
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
          <div className={styles.exportModal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.exportModalHeaderRight}>
              <button className={styles.exportHeaderBtn} onClick={() => setShowSettings(!showSettings)} style={{ background: showSettings ? '#f1f5f9' : '#fff' }}>
                ✏️ កំណត់ក្បាលឯកសារ
              </button>
              ...
              <button className={styles.exportHeaderBtn} onClick={handleTelegram} disabled={tgSending} style={{ color: '#0ea5e9', borderColor: '#0ea5e9', opacity: tgSending ? 0.7 : 1 }}>

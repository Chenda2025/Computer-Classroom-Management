...
const COLUMNS = [
  ...
  { key: "grade", label: "និទ្ទេស" },
  { key: "result", label: "លទ្ធផល" },
  { key: "promoted", label: "វគ្គបន្ទាប់" },
  { key: "date", label: "ថ្ងៃប្រឡង" },
] as const;
...
function cellValue(r: ExamResultRow, key: ColKey, idx: number): string {
  const passed = r.score >= r.exam.passingScore;
  switch (key) {
    case "index":
      return String(idx + 1);
    case "studentCode":
      return r.student.studentCode;
    case "name":
    ...
    case "promoted":
      return r.promoted ? "បានបន្តវគ្គ" : "—";
    case "date":
      return new Date(r.createdAt).toLocaleDateString("km-KH", {
        year: "numeric",
        month: "short",
        day: "numeric",
      });
...
export default function ExportModal({ results, onClose }: Props) {
  ...
  const [mainTitle, setMainTitle] = useState("របាយការណ៍លទ្ធផលប្រឡង");
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Note: we don't restore mainTitle for reports since it should be specific to this report, but user can customize
        ...
        if (data.signRightRole !== undefined)
          setSignRightRole(data.signRightRole);
        else if (data.signRole !== undefined) setSignRightRole(data.signRole);
        if (data.autoLunarDate !== undefined)
          setAutoLunarDate(data.autoLunarDate);
        if (data.autoSolarDate !== undefined)
          setAutoSolarDate(data.autoSolarDate);
      } catch (e) {}
    }
    setLoaded(true);
  }, []);
  ...
  useEffect(() => {
    if (autoLunarDate.includes(".......................")) {
      import("khmer-chhankitek-calendar")
        .then((m) => {
          try {
  ...
  const buildAoa = () => [
    ...
    ...results.map((r, i) => activeCols.map((c) => cellValue(r, c.key, i))),
    [],
    ["សរុបលទ្ធផល៖ " + results.length + " នាក់"],
    [],
  ...
  const handleDocx = async () => {
    ...
    const headerParas = [
      new Paragraph({
        ...
        children: [new TextRun({ text: rightLine1, bold: true, size: 24 })],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [new TextRun({ text: rightLine2, bold: true, size: 20 })],
      }),
      new Paragraph({ text: "" }),
      new Paragraph({
        alignment: AlignmentType.LEFT,
        children: [new TextRun({ text: leftLine1, bold: true, size: 20 })],
      }),
      new Paragraph({
        alignment: AlignmentType.LEFT,
        children: [new TextRun({ text: leftLine2, size: 20 })],
      }),
      new Paragraph({
        alignment: AlignmentType.LEFT,
        children: [new TextRun({ text: leftLine3, size: 20 })],
      }),
      new Paragraph({ text: "" }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
          ...
          (r, i) =>
            new TableRow({
              children: activeCols.map(
                (c) =>
                  new TableCell({
                    children: [new Paragraph({ text: cellValue(r, c.key, i) })],
                  }),
              ),
            }),
        ),
    ...
    const doc = new Document({
      sections: [{ children: [...headerParas, table] }],
    });
              ...
              >
                ...
                >
                  <div
                    style={{
                      fontFamily:
                        '"Moul", "Khmer OS Muol Light", "Khmer OS Muol"',
                      fontSize: "1.1em",
                    }}
                  >
                    {leftLine1}
                ...
                >
                  ...
                  {rightLine3 && (
                    <div
                      style={{
                        fontFamily: 'Tacteing, Tacteng, "Khmer OS Tacteng"',
                        fontSize: "1.5em",
                        marginTop: 8,
                      }}
                    >
                  ...
                  >
                    <div
                      style={{
                        ...
                        marginTop: 16,
                        fontFamily:
                          'var(--font-battambang), "Khmer OS Battambang"',
                        color: "var(--color-primary, #2563eb)",
                      }}
                    >
                      <p style={{ margin: "0 0 4px" }}>
                        សរុប <strong>{results.length}</strong> លទ្ធផល
                      </p>
                    </div>
                    <div
                      style={{
                      ...
                      >
                        ...
                        <p
                          style={{
                            margin: "0",
                            fontFamily:
                              'var(--font-moul), "Khmer OS Muol Light", "Khmer OS Muol"',
                            whiteSpace: "pre-wrap",
                          }}
                        >
                          {signLeftRole}
              ...
              <div className={styles.exportBody}>
                    ...
                    >
                      <div className={styles.settingsModalHeader}>
                        <div>
                          ...
                          >
                            ផ្លាស់ប្ដូរអត្ថបទសម្រាប់ក្បាលខាងលើ និងចំណងជើង
                          </p>
                        </div>
                        <button
                        ...
                        <div className={styles.settingsRow}>
                          ...
                          <div className={styles.settingsSection}>
                            ...
                            <div>
                              ...
                              >
                                ជួរទី៣ (បញ្ចូលនិមិត្តសញ្ញា)
                              </label>
                              <input
                                className={styles.input}
                                style={{
                                  fontFamily:
                                    'Tacteing, Tacteng, "Khmer OS Tacteng"',
                                  fontSize: "1.2rem",
                                }}
                                value={rightLine3}
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <div>
                                ...
                                <textarea
                                  ...
                                  value={signLeftRole}
                                  onChange={(e) =>
                                    setSignLeftRole(e.target.value)
                                  }
                                />
                              </div>
                            </div>
                            <div style={{ flex: 1 }}>
                              ...
                              <div>
                                ...
                                <textarea
                                  ...
                                  value={signRightRole}
                                  onChange={(e) =>
                                    setSignRightRole(e.target.value)
                                  }
                                />
                              </div>
                            </div>
                          ...
                          >
                            <div style={{ flex: 1 }}>
                              ...
                              <input
                                ...
                                value={autoLunarDate}
                                onChange={(e) =>
                                  setAutoLunarDate(e.target.value)
                                }
                              />
                            </div>
                            <div style={{ flex: 1 }}>
                              ...
                              <input
                                ...
                                value={autoSolarDate}
                                onChange={(e) =>
                                  setAutoSolarDate(e.target.value)
                                }
                              />
                            </div>
                          </div>
                ...
                <div className={styles.exportPreview}>
                  <div className={styles.previewPaper}>
                    <div
                      className={styles.previewHeaderBlock}

...
const COLUMNS = [
  ...
  { key: 'wat', label: 'វត្ត' },
  { key: 'kuti', label: 'កុដិ' },
  { key: 'educationLevel', label: 'កម្រិតសិក្សា' },
  { key: 'grade', label: 'ថ្នាក់ទី' },
  { key: 'course', label: 'វគ្គសិក្សា' },
...
function cellValue(s: Student, key: ColKey, idx: number, courses: { id: string; name: string }[]): string {
  switch (key) {
    case 'index': return String(idx + 1);
    case 'photo': return s.photoUrl ? 'មានរូប' : 'គ្មានរូប';
    case 'gender': return s.gender === 'M' ? 'ប្រុស' : s.gender === 'F' ? 'ស្រី' : '—';
    case 'status': return s.gender && s.dateOfBirth && s.wat ? 'ពេញ' : 'មិនពេញ';
    case 'createdAt': return new Date(s.createdAt).toLocaleDateString('km-KH', { year: 'numeric', month: 'short', day: 'numeric' });
    case 'course':
      if (!s.enrollments || s.enrollments.length === 0) return '—';
      return s.enrollments.map(e => courses.find(c => c.id === e.courseId)?.name || 'Unknown').join(', ');
...
export default function ExportModal({ students, courses, onClose }: Props) {
  ...
  const [mainTitle, setMainTitle] = useState('បញ្ជីឈ្មោះសិស្សសរុប');
  ...
  const buildAoa = () => [
    ...
    ...students.map((s, i) => activeCols.map(c => cellValue(s, c.key, i, courses))),
    [],
    ...
    const table = new Table({
      ...
      rows: [
        ...
        ...students.map((s, i) =>
          new TableRow({
            children: activeCols.map(c =>
              new TableCell({ children: [new Paragraph({ text: cellValue(s, c.key, i, courses) })] })
            ),
          })
        ),
      ],

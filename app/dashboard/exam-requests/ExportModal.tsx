...
const COLUMNS = [
  { key: 'index', label: '#' },
  { key: 'studentCode', label: 'លេខកូដ' },
  { key: 'name', label: 'ឈ្មោះ' },
  { key: 'gender', label: 'ភេទ' },
  { key: 'examTitle', label: 'ប្រឡង' },
  { key: 'courseName', label: 'វគ្គសិក្សា' },
  { key: 'status', label: 'ស្ថានភាព' },
  { key: 'note', label: 'ចំណាំ' },
] as const;
...
function cellValue(
  ...
  switch (key) {
    ...
    case 'studentCode': return r.student.studentCode;
    case 'name': return r.student.name;
    case 'gender': return r.student.gender === 'M' ? 'ប្រុស' : r.student.gender === 'F' ? 'ស្រី' : '—';
    case 'examTitle': return r.exam.title;
    case 'courseName': return r.exam.course.name;
...
export default function ExportModal({ requests, onClose }: Props) {
  ...
  const [mainTitle, setMainTitle] = useState('របាយការណ៍ស្នើសុំប្រឡង');
  const [subTitle, setSubTitle] = useState('សរុបសិស្សទាំងអស់');
  ...
  const [tgTextSending, setTgTextSending] = useState(false);
  const [tgTextStatus, setTgTextStatus] = useState<'idle' | 'ok' | 'error'>('idle');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('exportSettings', JSON.stringify(data));
  ...
  const buildAoa = () => [
    ...
    ...requests.map((r, i) => activeCols.map(c => cellValue(r, c.key, i))),
    [],
    ['សរុបអ្នកចូលរួម៖ ' + requests.length + ' នាក់'],
    [],
  ...
  const handleExcel = async () => {
    ...
    XLSX.writeFile(wb, 'exam_requests.xlsx');
  };
    ...
    const table = new Table({
      ...
      rows: [
        ...
        ...requests.map((r, i) =>
          new TableRow({
            children: activeCols.map(c =>
              new TableCell({ children: [new Paragraph({ text: cellValue(r, c.key, i) })] })
            ),
          })
        ),
      ],
  ...
  const handleTelegramText = async () => {
    setTgTextSending(true);
    setTgTextStatus('idle');
    try {
      let approvedCount = 0;
      let pendingCount = 0;
      let rejectedCount = 0;
      ...
      requests.forEach(r => {
        if (r.status === 'APPROVED') approvedCount++;
          ...
          pendingCount++;
          pendingList.push(`${r.student.studentCode} - ${r.student.name} - ${r.exam.title}`);
        }
      });

      let text = `📅 <b>${mainTitle}</b>\n`;
      text += `${subTitle}\n`;
      text += `📆 កាលបរិច្ឆេទ៖ ${new Date().toLocaleDateString('km-KH', { year: 'numeric', month: 'long', day: 'numeric' })}\n\n`;
      
      text += `👥 សំណើសរុប៖ ${requests.length}\n`;
      text += `✅ អនុម័តរួច៖ ${approvedCount}\n`;
      text += `⏳ រង់ចាំ៖ ${pendingCount}\n`;
      if (rejectedCount > 0) text += `❌ បដិសេធ៖ ${rejectedCount}\n`;

      if (pendingList.length > 0) {
        text += `\n📝 <b>បញ្ជីឈ្មោះសិស្សកំពុងរង់ចាំ៖</b>\n`;
        pendingList.forEach((line, i) => {
          text += `${i + 1}. ${line}\n`;
        });
      }
      ...
      setTgError(err.message);
      setTgTextStatus('error');
    } finally {
      setTgTextSending(false);
    }
  };
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85em' }}>
            ...
            <tbody>
              {requests.map((r, i) => (
                <tr key={r.id}>
                  {activeCols.map(c => (
                    <td key={c.key} style={{ border: '1px solid #333', padding: '6px 8px', textAlign: 'left', verticalAlign: 'middle', color: '#000691' }}>
                      {cellValue(r, c.key, i)}
                    </td>
                  ))}
                </tr>
              ...
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{requests.length}</strong> នាក់</p>
                  </div>
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
          <div className={styles.exportModal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.exportModalHeaderRight}>
              ...
              <button
                className={styles.exportHeaderBtn}
                onClick={handleTelegramText}
                disabled={tgTextSending}
                style={{ color: '#0ea5e9', borderColor: '#0ea5e9', opacity: tgTextSending ? 0.7 : 1 }}
              >
                {tgTextSending ? 'កំពុងផ្ញើ...' : tgTextStatus === 'ok' ? '✓ បានផ្ញើអត្ថបទ' : tgTextStatus === 'error' ? '✗ បរាជ័យ' : '✈️ Telegram (អត្ថបទ)'}
              </button>
              <button
                className={styles.exportHeaderBtn}
              ...
              >
                {tgSending ? 'កំពុងផ្ញើ...' : tgStatus === 'ok' ? '✓ បានផ្ញើរូបភាព' : tgStatus === 'error' ? '✗ បរាជ័យ' : '✈️ Telegram (រូបភាព)'}
              </button>
                  ...
                  <table className={styles.previewTable}>
                    ...
                    <tbody>
                      {requests.map((r, i) => (
                        <tr key={r.id}>
                          {activeCols.map(c => (
                            <td key={c.key} style={{ padding: '8px 12px', borderBottom: '1px solid var(--color-border)', color: 'var(--color-text-secondary)' }}>
                              {cellValue(r, c.key, i)}
                            </td>
                          ))}
                        </tr>

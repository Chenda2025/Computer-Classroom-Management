...
const COLUMNS = [
  ...
  { key: 'status', label: 'វត្តមានថ្ងៃនេះ' },
  { key: 'absentCount', label: 'អវត្តមានសរុប (ខែនេះ)' },
  { key: 'permissionCount', label: 'ច្បាប់សរុប (ខែនេះ)' },
] as const;
...
function cellValue(
  ...
  monthlyCounts: Record<string, { absent: number; permission: number }>
): string {
  switch (key) {
    case 'index': return String(idx + 1);
    case 'studentCode': return s.studentCode;
    case 'name': return s.name;
    case 'gender': return s.gender === 'M' ? 'ប្រុស' : s.gender === 'F' ? 'ស្រី' : '—';
    case 'status': return STATUS_LABELS[statusMap[s.id] || 'PRESENT'] || 'វត្តមាន';
    case 'absentCount': return String(monthlyCounts[s.id]?.absent || 0);
    case 'permissionCount': return String(monthlyCounts[s.id]?.permission || 0);
    default: return '—';
  }
}
...
export default function ExportModal({ students, courseName, date, statusMap, monthlyCounts, onClose }: Props) {
  ...
  const buildAoa = () => [
    ...
    ...students.map((s, i) => activeCols.map(c => cellValue(s, c.key, i, statusMap, monthlyCounts))),
    [],
    ['សរុបអ្នកចូលរួម៖ ' + students.length + ' នាក់'],
    [],
    ...
    const table = new Table({
      ...
      rows: [
        ...
        ...students.map((s, i) =>
          new TableRow({
            children: activeCols.map(c =>
              new TableCell({ children: [new Paragraph({ text: cellValue(s, c.key, i, statusMap, monthlyCounts) })] })
            ),
          })
        ),
      ],
  ...
  const handleTelegram = async () => {
    ...
    try {
      ...
      fd.append('file', blob);
      fd.append('filename', 'students.png');
  ...
  const handleTelegramText = async () => {
    ...
    try {
      ...
      text += `👥 សិស្សសរុប៖ ${students.length} នាក់\n`;
      text += `✅ វត្តមាន៖ ${presentCount} នាក់\n`;
      text += `❌ អវត្តមាន៖ ${absentCount} នាក់\n`;
                ...
                <tr key={s.id}>
                  {activeCols.map(c => (
                    <td key={c.key} style={{ border: '1px solid #333', padding: '6px 8px', textAlign: 'left', verticalAlign: 'middle', color: '#000691' }}>
                      {cellValue(s, c.key, i, statusMap, monthlyCounts)}
                    </td>
                  ))}
                </tr>
              ))}
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{students.length}</strong> នាក់</p>
                  </div>

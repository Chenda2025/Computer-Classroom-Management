...
const COLUMNS = [
  { key: 'index', label: '#' },
  { key: 'name', label: 'ឈ្មោះវគ្គសិក្សា' },
  { key: 'description', label: 'ការពិពណ៌នា' },
  { key: 'enrollments', label: 'សិស្សចុះឈ្មោះ' },
  { key: 'lessonPlans', label: 'កិច្ចតែងការ' },
  { key: 'exams', label: 'ការប្រឡង' },
  { key: 'createdAt', label: 'កាលបរិច្ឆេទបង្កើត' },
] as const;
...
function cellValue(c: Course, key: ColKey, idx: number): string {
  switch (key) {
    case 'index': return String(idx + 1);
    case 'enrollments': return String(c._count.enrollments);
    case 'lessonPlans': return String(c._count.lessonPlans);
    case 'exams': return String(c._count.exams);
    case 'createdAt': return new Date(c.createdAt).toLocaleDateString('km-KH', { year: 'numeric', month: 'short', day: 'numeric' });
...
export default function ExportModal({ courses, onClose }: Props) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('exportSettings_courses');
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('exportSettings_courses', JSON.stringify(data));
  ...
  const buildAoa = () => [
    ...
    ...courses.map((s, i) => activeCols.map(c => cellValue(s, c.key, i))),
    [],
    ['សរុបវគ្គសិក្សា៖ ' + courses.length + ' វគ្គ'],
    [],
              ...
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{courses.length}</strong> វគ្គ</p>
                  </div>
              ...
              <div className={styles.exportModalHeaderLeft}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)' }}>
                    វគ្គសិក្សាសរុប៖ {courses.length} វគ្គ
                  </span>
                </div>
              </div>
                    ...
                    <table className={styles.previewTable}>
                      ...
                      <tbody>
                        {courses.map((s, i) => (
                          <tr key={s.id}>
                            {activeCols.map(c => (
                              <td key={c.key} style={{ padding: '8px 12px', borderBottom: '1px solid var(--color-border)', color: 'var(--color-text-secondary)', textAlign: 'left' }}>
                                {cellValue(s, c.key, i)}
                              </td>
                            ))}
                          </tr>

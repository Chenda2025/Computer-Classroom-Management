...
const COLUMNS = [
  ...
  { key: 'issuedDate', label: 'ថ្ងៃចេញ' },
  { key: 'description', label: 'ការពិពណ៌នា' },
] as const;

type ColKey = (typeof COLUMNS)[number]['key'];

interface StudentRef { id: string; studentCode: string; name: string; }
interface Certificate {
  id: string; title: string; issuedDate: string; description: string | null;
  studentId: string; student: StudentRef; createdAt: string; updatedAt: string;
}
...
function cellValue(
  ...
  switch (key) {
    ...
    case 'studentCode': return c.student.studentCode;
    case 'name': return c.student.name;
    case 'title': return c.title;
    case 'issuedDate': return c.issuedDate;
...
export default function ExportModal({ data, onClose }: Props) {
  ...
  const buildAoa = () => [
    ...
    ...data.map((c, i) => activeCols.map(col => cellValue(c, col.key, i))),
    [],
    ['សរុប៖ ' + data.length + ' វិញ្ញាបនបត្រ'],
    [],
  ...
  const handlePdf = async () => {
    try {
      ...
      const opt = {
        ...
        filename:     'certificates_report.pdf',
  ...
  const handleDocx = async () => {
    ...
    a.download = 'certificates_report.docx';
    a.click();
  };
  ...
  const handleTelegram = async () => {
    ...
    try {
      ...
      fd.append('file', blob);
      fd.append('filename', 'certificates_report.png');
  ...
  const handleTelegramText = async () => {
    ...
    try {
      ...
      text += `📜 សរុប៖ ${data.length} វិញ្ញាបនបត្រ\n`;

      if (data.length > 0) {
        text += `\n📝 <b>បញ្ជីឈ្មោះសិស្សទទួលបានវិញ្ញាបនបត្រ៖</b>\n`;
        data.forEach((c, i) => {
          text += `${i + 1}. ${c.student.studentCode} - ${c.student.name} - ${c.title}\n`;
        });
      }
              ...
              <tr style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <td colSpan={activeCols.length} style={{ border: 'none', padding: 0 }}>
                  <div style={{ fontSize: '0.85em', marginTop: 16, fontFamily: 'var(--font-battambang), "Khmer OS Battambang"', color: 'var(--color-primary, #2563eb)' }}>
                    <p style={{ margin: '0 0 4px' }}>សរុប <strong>{data.length}</strong> វិញ្ញាបនបត្រ</p>
                  </div>
